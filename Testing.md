...
| 2 | Test character movement (Space key press) | UFO bounces upwards on each key press. | 
| 3 | Test collision between character and top alien | Game Over screen is displayed. | 
| 4 | Test collision between character and bottom alien | Game Over screen is displayed. | 
| 5 | Test collision with the top of the game window (ceiling) | Game Over screen is displayed. | 
| 6 | Test collision between character and bottom of game window | Game Over screen is displayed. |
| 7 | Test score increment | Score increases by one every second while the game is running. |
| 8 | UFO animation points upwards | When the Space key is pressed, the UFO tilts upwards slightly. | 
| 9 | Welcome sound test | Welcome sound plays when the game launches. | 
| 10 | Collision sound test | Explosion sound plays when a collision occurs. | 
| 11 | Space key pressed on Start screen | Game starts and continues to the gameplay screen. |
| 12 | Space key pressed on Game Over screen | Game restarts and returns to the gameplay screen. | 
| 13 | Start screen game launch | Start screen appears when the game launches. | 
| 14 | Alien height during gameplay | Aliens spawn at different heights chosen randomly from a predefined list. | 
| 15 | Alien gap during gameplay | Gap between alien pairs is selected randomly from the available values. | 
| 16 | Random UFO selection | One of four UFO sprites is selected randomly each time a new game starts. | 
| 17 | UFO animation points downwards | When the Space key is not being pressed, the UFO points downwards. |
| 18 | Game Over collision test | Game Over screen is displayed after a collision. | 
